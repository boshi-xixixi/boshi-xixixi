<!--
用法：新建一个公开仓库，仓库名必须和你的用户名一模一样：boshi-xixixi/boshi-xixixi
把这个文件内容作为该仓库的 README.md，GitHub 就会把它渲染到你的个人主页顶部。
所有内容均依据你的简历填写，如需调整自行微调即可。
-->

<div align="center">

# 你好，我是李世博 👋

[![Typing SVG](https://readme-typing-svg.demolab.com?font=Fira+Code&size=22&pause=1000&color=36BCF7&center=true&vCenter=true&width=620&lines=AI+产品经理+·+用代码验证产品想法;TraeSkill+作者+·+250%2B+Star;数据科学与大数据+本科在读;字节+Trae+大连黑客松+冠军+%F0%9F%8F%86;从中职到本科+三段学业专业前三)](https://github.com/boshi-xixixi)

[![Email](https://img.shields.io/badge/邮箱-755508842%40qq.com-red?logo=maildotru&logoColor=white)](mailto:755508842@qq.com)
![Location](https://img.shields.io/badge/📍大连-Live-purple)
![MBTI](https://img.shields.io/badge/MBTI-INFJ-blueviolet)

</div>

## 🧑‍💻 关于我

- 🎓 **辽宁科技学院 · 数据科学与大数据技术** 本科在读（2025 - 2027）；中职（专业 1/193）→ 高职（专业 1/154，GPA 4.25）→ 本科（专业 3/106），三段学业均在专业前列
- 🤖 **AI 产品方向**：开源项目 [TraeSkill](https://github.com/boshi-xixixi/TraeSkill) 作者（**250+ Star / 60+ Fork**）；Trae 官方飞书社群 & 论坛管理者
- 🏆 **常驻 AI 编程赛场**：字节 Trae 大连黑客松 🥇、全国职业院校技能大赛二等奖、百度秒哒全国二等奖、美团黑客松全国前 30
- 🛠️ 独立完成从产品设计到全栈开发落地：Python / Flask、Java、Flutter、Vue、云服务器部署、小程序

## 🏆 竞赛与荣誉

| 年份 | 赛事 | 成绩 |
| --- | --- | --- |
| 2025 | 字节跳动 Trae 大连黑客松 | 🥇 **第一名** |
| 2023 | 全国职业院校技能大赛 · 中职组移动应用开发 | 🥈 团体二等奖（国赛） |
| 2024 | 辽宁省职业技能大赛 · 移动应用开发 | 🥈 个人银牌 |
| 2025 | 百度秒哒非程序员大赛 | 🥈 全国二等奖 |
| 2026 | 美团黑客松 | 🏅 全国前 30 |
| 2025 | 长风杯全国大学生大数据分析与挖掘竞赛 | 🥉 三等奖 |
| 2025 | 豆包应用生成创作挑战赛 | 🏅 全国前 200 |
| 2026 | 字节 Trae AI 创作力大赛 | 🏅 数万作品中全国前 350 |

🎖️ **在校荣誉**：大连市奖学金 ×3 · 辽宁省高等学校优秀毕业生 · 计算机程序设计员三级（高级工）· 1+X Web 前端 / Python 数据应用开发等级证书

## 🛠️ 技术栈

<div align="center">

<a href="https://skillicons.dev">
  <img src="https://skillicons.dev/icons?i=python,flask,java,ts,vue,js,flutter,html,css,mysql,linux,git&perline=12" />
</a>

**也在用**：Trae / AI Agent 工作流 · Axure / XD（UI/UX）· 小程序开发 · Hadoop 大数据基础

</div>

## 🚀 精选项目

| 项目 | 简介 | 亮点 |
| --- | --- | --- |
| [TraeSkill](https://github.com/boshi-xixixi/TraeSkill) | 为 Trae IDE 打造的全流程标准化 AI 开发技能库，内置调度分发与多角色研发技能，一键导入规范 AI 编码工作流 | ⭐ 250+ / 🍴 60+ |
| [稳过](https://wenguo.shawu.chat) | 面向大学生的 AI 刷题备考平台：一键上传 Word 习题自动生成结构化题库，覆盖在线做题、错题沉淀、AI 专项练习全链路 | Trae AI 创作力大赛全国前 350 · 独立完成产品设计到全栈开发 |
| [AI 小画家](https://painter.shawu.chat) | 0-12 岁儿童智能绘画教育平台，绘画创作 × 语言学习 × 认知发展，多模态 AI 全程引导 | 枫叶优体杯互联网+儿童大赛 三等奖 |
| [jd-mate-ai](https://github.com/boshi-xixixi/jd-mate-ai) | 粘贴 JD 自动生成定制面试题、简历优化建议与模拟面试 | 字节 Trae 大连黑客松 🥇 作品 |

## 📊 GitHub 数据

<div align="center">

<img height="170" src="https://github-readme-stats.vercel.app/api?username=boshi-xixixi&show_icons=true&theme=radical&count_private=true&include_all_commits=true&cache_bust=20260902" alt="GitHub Stats" />
<img height="170" src="https://github-readme-stats.vercel.app/api/top-langs/?username=boshi-xixixi&layout=compact&theme=radical&langs_count=8&hide_border=false&cache_bust=20260902" alt="Top Langs" />

<img src="https://streak-stats.demolab.com?user=boshi-xixixi&theme=radical&locale=zh_Hans&short_numbers=true" alt="GitHub Streak" />

[![Trophy](https://github-profile-trophy.vercel.app/?username=boshi-xixixi&row=1&column=8&margin-w=8&theme=radical&no-frame=true&cache_bust=20260902)](https://github.com/boshi-xixixi)

<img src="https://cdn.jsdelivr.net/gh/boshi-xixixi/boshi-xixixi@output/github-contribution-grid-snake.svg" alt="Snake animation" />

</div>

## 📮 联系我

<div align="center">

📧 [755508842@qq.com](mailto:755508842@qq.com)

**“从中职一路走到本科，用作品说话。”** ✨

</div>
